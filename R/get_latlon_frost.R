...
#'
#' @param stationid A station number as integer and defined by met.no, default is 18700 - Blindern
#' @param paramid A parameter number as integer and defined by met.no, default is NULL, thus getting all paramid available
#'
...
get_latlon_frost <- function(stationid = 18700,
  ...
  # Libraries
  require(sf) # utm conversion
  require(httr) # fetch frost metadata
  ...
  stn <- st_as_sf(stn_attrib,coords=c("lon","lat"),crs=st_crs(4326)) %>% st_transform(25833) #32633
