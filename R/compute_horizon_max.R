...
#' compute_horizon_max(stn, dx, resx, step=1, f.plot.polygon=FALSE)
#'
#' @export
...
compute_horizon_max <- function(stn = NULL,
  ...
  horizon_dem   <- compute_horizon(stn.centre,
                                   ...
                                   level = stn.level,
                                   step = step,
                                   f.plot.polygon = f.plot.polygon)
  horizon_dsm   <- compute_horizon(stn.centre,
                                   dsm,
                                   level = stn.level,
                                   step = step,
                                   f.plot.polygon = f.plot.polygon)
  horizon_demkm <- compute_horizon(stn.centre,
                                   demkm,
                                   level = stn.level,
                                   step = step,
                                   f.plot.polygon = f.plot.polygon)
