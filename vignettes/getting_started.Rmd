...
```{r}
stn
as.data.frame(stn[c("dx","resx","path")])
```
...
plot_station_grid(stn, tile_name = "ortofoto", grid_scale = 10)
```
...
# Plot a sun diagram
The function `plot_station_horizon_sun()` computes and plots sun diagram and requires the following input:
